...
def data_loader_custm(dataset, datapath, data_transforms, imb_factor):
    print(f"dataset: {dataset}, datapath: {datapath}, imb_factor: {imb_factor}")
    
    if imb_factor == 0.01:
        train_subdir = "CIFAR10_LT001" if dataset == "CIFAR10" else "CIFAR100_LT001"
    elif imb_factor == 0.1:
        train_subdir = "CIFAR10_LT01" if dataset == "CIFAR10" else "CIFAR100_LT01"
    else:
        raise Exception(f"Unsupported imbalance factor: {imb_factor}")
    
    train_datapath = os.path.join(datapath, train_subdir)
    print(f"training set path: {train_datapath}")
    
    if not os.path.exists(train_datapath):
        raise Exception(f"Trainingset path does not exist: {train_datapath}")
    
    image_extensions = ['*.png', '*.jpg', '*.jpeg']
    data_list_trian = []
    for ext in image_extensions:
        pattern = os.path.join(train_datapath, "*", ext)
        files = glob.glob(pattern)
        data_list_trian.extend(files)
        
    

    test_subdir = "TEST_CIFAR10" if dataset == "CIFAR10" else "TEST_CIFAR100"
    test_datapath = os.path.join(datapath, test_subdir)
    print(f"Testset path: {test_datapath}")
    
    if not os.path.exists(test_datapath):
        raise Exception(f"Testset path does not exist: {test_datapath}")
    
    data_list_test = []
    for ext in image_extensions:
        pattern = os.path.join(test_datapath, "*", ext)
        files = glob.glob(pattern)
        data_list_test.extend(files)
    
    
    if dataset == 'CIFAR10':
        fixed_classes = ['airplane', 'automobile', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck']
        label_dict = {}
        for i, class_name in enumerate(fixed_classes):
            label_dict[class_name] = i
        print(f"CIFAR-10 Label dictionary: {label_dict}")
    elif dataset == 'CIFAR100':
        label_names = sorted([d for d in os.listdir(train_datapath) if os.path.isdir(os.path.join(train_datapath, d))])
        label_dict = {}
        for i, class_name in enumerate(label_names):
            label_dict[class_name] = i
        print(f"CIFAR-100 Label dictionary: {label_dict}")

    # ------------------Load Original CIFAR10/100 data------------------------#
    if dataset == 'CIFAR10' and imb_factor == 0.01:
        train_dataset = MyDataset(data_list_trian, transform=data_transforms['train'], label_dict=label_dict)
        val_dataset = dset.CIFAR10(root="./DATASET", train=False, download=True, transform=data_transforms['test'])
        test_dataset = MyDataset(data_list_test, transform=data_transforms['test'], label_dict=label_dict)
