...
class Block(nn.Module):
    def __init__(self, dim, dim_out, groups = 8):
        super().__init__()
        self.proj = nn.Conv1d(dim, dim_out, 1, padding = 0)
...
class UNet_conditional(nn.Module):
    def __init__(
        ...
        if dim == 2048:
            self.fc_256in = nn.Linear(dim, dim//4)
            init_dim = dim//4
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                Downsample(dim_in, dim_out) if not is_last else nn.Conv1d(dim_in, dim_out, 1, padding = 0)
            ]))
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.ups.append(nn.ModuleList([
                ...
                Upsample(dim_out, dim_in) if not is_last else  nn.Conv1d(dim_out, dim_in, 1, padding = 0)
            ]))
        ...
        if dim == 2048:
            ...
            self.final_out256 = nn.Linear(init_dim, init_dim * 4)
        else:
...
class ConditionalDiffusion1D(nn.Module):
    ...
    def model_predictions(self, x, t, label, x_self_cond = None, clip_x_start = False, rederive_pred_noise = False):
        model_output = self.model(x, t, label, x_self_cond)
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, label, x_self_cond = None, clip_denoised = True):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((b,), t, device = x.device, dtype = torch.long)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x = x, t = batched_times, label = label, x_self_cond = x_self_cond, clip_denoised = clip_denoised)
    ...
    def sample(self, label, batch_size = 16,):
        ...
        return sample_fn(label, (batch_size, channels, seq_length))
