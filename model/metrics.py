...
def get_mmf_idx(img_num: int):
    """
    获取 Many/Medium/Few Shot 索引
    返回: 0 (Many), 1 (Medium), 2 (Few)
    
    修复: 将 Few Shot 阈值从 20 调整为 60，以适配 CIFAR-10-LT-0.01 (最小 50 samples)
    """
    assert type(img_num) == int
    if img_num < 60:  # Few Shot: < 60 samples
        return 2
    elif img_num < 100:  # Medium Shot: 60-99 samples
        return 1
    else:  # Many Shot: >= 100 samples
        return 0
